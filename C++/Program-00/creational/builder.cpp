...
public:
    ...
    BurgerBuilder(int size)
        : size(size), cheese(false), pepperoni(false), lettuce(false), tomato(false)
    {
    }
...
public:
    Burger(BurgerBuilder *builder)
        : cheese(builder->cheese), pepperoni(builder->pepperoni), lettuce(builder->lettuce), tomato(builder->tomato)
    {
    }
};
...
int main(void)
{
    std::shared_ptr<Burger> burger = BurgerBuilder(14)
                                         .addPepperoni()
                                         .addLettuce()
                                         .addTomato()
                                         .build();
